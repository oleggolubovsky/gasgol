    ...
    >
      ...
      <template v-slot:item.name="{ item }">
        <v-btn
          ...
          text
          :to="{ path: '/messaging-campaigns/' + (item.id || 0) }"
        >
          {{ item.name }}
        </v-btn>
      </template>
      <template v-slot:item.edit="{ item }">
        <v-btn color="primary" fab x-small outlined :to="{ path: '/messaging-campaigns/' + (item.id || 0) }">
...
export default class DataTable extends Vue {
  ...
  @Watch('fetchDataParams.params.campaignId')
