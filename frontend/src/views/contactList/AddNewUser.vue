...
export default class AddNewUser extends Vue {
  ...
  create() {
    ...
    if (this.contactLists1.enable) {
      contactListIds.push(this.contactLists1.id);
    }
  ...
  fetchData(params: ContactListParams) {
    ...
    return axiosWrap({
      ...
      .then((res: any) => {
        this.contactLists = this.getContactLists(res.contactList);
        this.name = res.contactName;
        const enabled = this.contactLists.findIndex((item: any) => item.id === window.newListParams.id);
  ...
  inputPhone(input: any) {
    if (input.length === 12) {
      this.fetchData({ phoneNumber: input });
    }
  }
}
Vue.use(VueTelInput)
