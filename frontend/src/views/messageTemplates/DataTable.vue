    ...
    >
      ...
      <template v-slot:item.name="{ item }">
        <v-btn
          ...
          text
          :to="{ path: '/contact-list/' + (item.id || 0) }"
        >
          {{ item.name }}
        </v-btn>
      </template>
      <template v-slot:item.status="{ item }">
        <v-icon :color="statusColor(item.status)">mdi-circle-medium</v-icon>
        {{ item.status }}
      </template>
      <template v-slot:item.edit="{ item }">
        <v-btn v-if="item.status" color="primary" fab x-small outlined :to="{ path: '/contact-list/' + (item.id || 0) }">
          <v-icon>mdi-pencil</v-icon>
        </v-btn>
        <v-btn v-else color="primary" fab x-small outlined>
...
export default class DataTable extends Vue {
  ...
  @Watch('fetchDataParams.params.templateId')
